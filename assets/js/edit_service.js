...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function validateServiceType(select) {
        ...
        if (value.length === 0) {
            setFieldState(select, 'invalid', 'Tip servisa je obavezan', errorDiv);
            return false;
        } else {
    ...
    function updateDaysAgo() {
        if (!daysAgoElement || !serviceDateInput) return;
        ...
        const differenceMs = today - selectedDate;
        const differenceDays = Math.floor(differenceMs / (1000 * 60 * 60 * 24));

        daysAgoElement.classList.remove('positive', 'negative', 'neutral');
        
        if (differenceDays === 0) {
            daysAgoElement.textContent = 'Danas';
            daysAgoElement.classList.add('neutral');
        } else if (differenceDays === 1) {
            daysAgoElement.textContent = 'Juče';
            daysAgoElement.classList.add('neutral');
        } else if (differenceDays < 7) {
            daysAgoElement.textContent = `${differenceDays} dana`;
            daysAgoElement.classList.add('neutral');
        } else if (differenceDays < 30) {
            const weeks = Math.floor(differenceDays / 7);
            daysAgoElement.textContent = `${weeks} nedelja`;
        } else if (differenceDays < 365) {
            const months = Math.floor(differenceDays / 30);
            daysAgoElement.textContent = `${months} meseci`;
        } else {
            const years = Math.floor(differenceDays / 365);
            daysAgoElement.textContent = `${years} godinu`;
        }
    }
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            if (isTypeValid && isDateValid && isDescriptionValid && isCostValid) {
                // Show loading state
    ...
    // ========== AUTO-SAVE DRAFT ==========
    function saveDraft() {
        if (serviceTypeSelect || serviceDateInput || descriptionTextarea || costInput) {
            const draft = {
            ...
            localStorage.setItem('edit_service_draft', JSON.stringify(draft));
        }
    }
    ...
    const savedDraft = localStorage.getItem('edit_service_draft');
    if (savedDraft) {
        try {
    ...
    if (form) {
        form.addEventListener('submit', function() {
            localStorage.removeItem('edit_service_draft');
        });
    }
    ...
    // ========== CONSOLE EASTER EGG ==========
    console.log('%c🔧 Edit Service Form', 'font-size: 20px; font-weight: bold; color: #f0ad4e;');
